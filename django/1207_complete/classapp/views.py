...
class RegisterView_S(FormView):
	...
	def post(self, request, *args, **kwargs): 
		...
		if form.is_valid():
			...
			try:
				student = models.Student.objects.create(email = new_email, password = new_pass_input, number = new_number_input)
	...
	def post(self,request,*args,**kwargs):
		...
		if form.is_valid():
			...
			if password == chatRoom.password:
				...
				return render(request, 'classapp/msg.html', {'msg':'비밀번호가 틀렸습니다','linl':reverse("classapp:chat_list")})
		else:
			return render(request,'classapp/chat_pass.html',{'form':form,'chat_num':chat_num})
